...
//gets sorted list of players on a team sorted by first name, just provide team name
	...
	//gets sorted list of players on a team sorted by position, just provide team name
	app.get('/team/players/Position', async (req, res) => {
    // obtain a connection from our pool of connections
    pool.getConnection(function (err, connection){
      if (err){
        console.log(connection);
        ...
        res.status(400).send('Problem obtaining MySQL connection'); 
			...
			var teamName = req.body.teamName;
            connection.query("SELECT FirstName,LastName,PlayerNumber FROM Players join Teams on Players.TeamID = Teams.TeamID WHERE TeamName=? ORDER BY Position",[teamName,fieldName], function (err, result, fields) {
              if (err) { 
                // if there is an error with the query, release the connection instance and log the error
                connection.release()
                logger.error("Problem getting list of players: ", err);
